...
ssh -o "StrictHostKeyChecking=no" $SSH_USER@$SSH_IP -p $SSH_PORT <<EOF
  ...
  cd /home/retyp
  docker-compose -f docker-compose.prod.yml stop retyp-api
  docker-compose -f docker-compose.prod.yml rm -f retyp-api
  docker-compose -f docker-compose.prod.yml up -d --no-deps --build retyp-api
EOF